...
for k,v in TMdict.items():
	if len(v)==1:
		for seq_record in SeqIO.parse("all_effector_wchap_genus_more100aa_CDHIT95_tmhmm.fasta", "fasta"):
			fasta_header = str(seq_record.id)
			fasta_sequence = str(seq_record.seq).upper()
			if k==fasta_header:
				with open("all_effector_wchap_genus_more100aa_CDHIT95_tmhmm_phobius.fasta", "a+") as output_file:
...
for seq_record in SeqIO.parse("notm_3reg_tmhmm_seq.fasta", "fasta"):
	...
	if fasta_header not in TMdict:
		with open("all_effector_wchap_genus_more100aa_CDHIT95_tmhmm_NOphobius.fasta", "a+") as output_file:
